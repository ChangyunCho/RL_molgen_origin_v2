...
def discriminator_net(ob,args,name='d_net'):
    with tf.compat.v1.variable_scope(name,reuse=tf.compat.v1.AUTO_REUSE):
        ...
        emb_node = tf.keras.layers.Dense(args.emb_size, activation=tf.nn.relu, use_bias=False, name='linear1')(emb_node)
        if args.bn==1:
            emb_node = tf.keras.layers.batch_normalization(emb_node,axis=-1)


        if args.gate_sum_d==1:
            emb_node_gate = tf.keras.layers.Dense(1,activation=tf.nn.sigmoid,name='gate')(emb_node)
        ...
        logit = tf.keras.layers.Dense(1, activation=None, name='linear2')(emb_graph)
        pred = tf.sigmoid(logit)
        # pred = tf.keras.layers.Dense(emb_graph, 1, activation=None, name='linear1')
        return pred,logit
...
class GCNPolicy(object):
    ...
    def _init(self, ob_space, ac_space, kind, atom_type_num,args):
        ...
        if args.bn==1:
            emb_stop = tf.keras.layers.batch_normalization(emb_stop,axis=-1)
        ...
        # self.logits_stop = tf.concat((tf.reduce_mean(tf.keras.layers.Dense(emb_node, 32, activation=tf.nn.relu, name='linear_stop1'),axis=1),tf.reshape(ob_len_first/5,[-1,1])),axis=1)
        # self.logits_stop = tf.keras.layers.Dense(self.logits_stop, 2, activation=None, name='linear_stop2')  # B*2
        ...
        if args.bn==1:
            self.vpred = tf.keras.layers.batch_normalization(self.vpred,axis=-1)
...
#
#         ### 2 predict stop
#         self.logits_stop = tf.reduce_sum(tf.keras.layers.Dense(emb_node, args.emb_size, activation=tf.nn.relu, name='linear_stop1'),axis=1)
#         self.logits_stop = tf.keras.layers.Dense(self.logits_stop, 2, activation=None, name='linear_stop2_1')  # B*2
#         # explicitly show node num
#         # self.logits_stop = tf.concat((tf.reduce_mean(tf.keras.layers.Dense(emb_node, 32, activation=tf.nn.relu, name='linear_stop1'),axis=1),tf.reshape(ob_len_first/5,[-1,1])),axis=1)
#         # self.logits_stop = tf.keras.layers.Dense(self.logits_stop, 2, activation=None, name='linear_stop2')  # B*2
#
...
#         ### 3.1: select first (active) node
#         # rules: only select effective nodes
#         self.logits_first = tf.keras.layers.Dense(emb_node, args.emb_size, activation=tf.nn.relu, name='linear_select1')
#         self.logits_first = tf.squeeze(tf.keras.layers.Dense(self.logits_first, 1, activation=None, name='linear_select2'),axis=-1) # B*n
#         logits_first_null = tf.ones(tf.shape(self.logits_first))*-1000
#         self.logits_first = tf.where(condition=logits_first_mask,x=self.logits_first,y=logits_first_null)
#         # using own prediction
#         pd_first = CategoricalPdType(-1).pdfromflat(flat=self.logits_first)
...
#         # mlp
#         emb_cat = tf.concat([tf.tile(emb_first,[1,tf.shape(emb_node)[1],1]),emb_node],axis=2)
#         self.logits_second = tf.keras.layers.Dense(emb_cat, args.emb_size, activation=tf.nn.relu, name='logits_second1')
#         self.logits_second = tf.keras.layers.Dense(self.logits_second, 1, activation=None, name='logits_second2')
#         # # bilinear
#         # self.logits_second = tf.transpose(bilinear(emb_first, emb_node, name='logits_second'), [0, 2, 1])
#
#         self.logits_second = tf.squeeze(self.logits_second, axis=-1)
...
#         # mlp
#         emb_cat = tf.concat([tf.tile(emb_first_real, [1, tf.shape(emb_node)[1], 1]), emb_node], axis=2)
#         self.logits_second_real = tf.keras.layers.Dense(emb_cat, args.emb_size, activation=tf.nn.relu, name='logits_second1',tf.compat.v1.get_variable)
#         self.logits_second_real = tf.keras.layers.Dense(self.logits_second_real, 1, activation=None, name='logits_second2')
#         # # bilinear
#         # self.logits_second_real = tf.transpose(bilinear(emb_first_real, emb_node, name='logits_second'), [0, 2, 1])
#
#         self.logits_second_real = tf.squeeze(self.logits_second_real, axis=-1)
...
#         # MLP
#         emb_cat = tf.concat([emb_first,emb_second],axis=-1)
#         self.logits_edge = tf.keras.layers.Dense(emb_cat, args.emb_size, activation=tf.nn.relu, name='logits_edge1')
#         self.logits_edge = tf.keras.layers.Dense(self.logits_edge, ob['adj'].get_shape()[1], activation=None, name='logits_edge2')
#         self.logits_edge = tf.squeeze(self.logits_edge,axis=1)
#         # # bilinear
#         # self.logits_edge = tf.reshape(bilinear_multi(emb_first,emb_second,out_dim=ob['adj'].get_shape()[1]),[-1,ob['adj'].get_shape()[1]])
#         pd_edge = CategoricalPdType(-1).pdfromflat(self.logits_edge)
...
#         # MLP
#         emb_cat = tf.concat([emb_first_real, emb_second_real], axis=-1)
#         self.logits_edge_real = tf.keras.layers.Dense(emb_cat, args.emb_size, activation=tf.nn.relu, name='logits_edge1')
#         self.logits_edge_real = tf.keras.layers.Dense(self.logits_edge_real, ob['adj'].get_shape()[1], activation=None,
#                                            name='logits_edge2
#         self.logits_edge_real = tf.squeeze(self.logits_edge_real, axis=1)
#         # # bilinear
#         # self.logits_edge_real = tf.reshape(bilinear_multi(emb_first_real, emb_second_real, out_dim=ob['adj'].get_shape()[1]),
...
#         # ncat_list = [tf.shape(logits_first),ob_space['adj'].shape[-1],ob_space['adj'].shape[0]]
#         self.pd = self.pdtype(-1).pdfromflat([self.logits_first,self.logits_second_real,self.logits_edge_real,self.logits_stop])
#         self.vpred = tf.keras.layers.Dense(emb_node, args.emb_size, activation=tf.nn.relu, name='value1')
#         self.vpred = tf.keras.layers.Dense(self.vpred, 1, activation=None, name='value2')
#         self.vpred = tf.reduce_max(self.vpred,axis=1)
#
#         self.state_in = []
#         self.state_out = []
