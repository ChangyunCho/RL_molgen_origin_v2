...
#     def sample(self):
#         u = tf.keras.initializers.RandomUniform(tf.shape(self.logits))
...
class CategoricalPd(Pd):
    ...
    def kl(self, other):
        a0 = self.logits - tf.math.reduce_max(self.logits, axis=-1, keepdims=True)
        a1 = other.logits - tf.math.reduce_max(other.logits, axis=-1, keepdims=True)
        ea0 = tf.exp(a0)
        ea1 = tf.exp(a1)
    ...
    def sample(self):
        ...
        return tf.math.argmax(self.logits - tf.math.log(-tf.math.log(u)), axis=-1)
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
...
class BernoulliPd(Pd):
    ...
    def sample(self):
        u = tf.keras.initializers.RandomUniform(tf.shape(self.ps))
